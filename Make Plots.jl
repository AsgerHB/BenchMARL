...
# ╔═╡ 0d49c422-6f47-11ef-0e33-6948a3d5cce9
begin
	using JSON
	using CSV
	using Plots
...
# ╔═╡ 03314935-57e7-4088-83e7-8f3010e305ef
isfile(fraction_safe_path)

# ╔═╡ 533fa153-3c8c-4590-866b-a96fb26afbb8
fraction_safe = CSV.read(fraction_safe_path, DataFrame, 
	header=[:episode, :fraction_safe])

# ╔═╡ 9ac46a71-ab48-468e-a9a3-5043e02dc497
md"""
!!! info "Total Frames? Steps?"
	I assume the "total frames" csv represent the number of frames trained on. Pretty intuitive naming I guess. In some of my other files I call these "episodes" or "runs" so I guess there is some naming confusion.
	(In BenchMARL terms, an episode is a collection of runs.)

	And since the numbers match, I assume the JSON entry "step_count" represents the same value by another name.
"""
...
md"""
!!! info "I just double-checked."
	`"agents_return"` a list of length `experiment.evaluation_episodes`.
	
	Each return value in the list is the mean sum of agent rewards for a 100-step rollout. That is, the local cost of each agent is calculated by summing up rewards over the entire episode. Then the mean is taken accross agents. To confirm this, cf. this line in the file `logger.py`:
